<template>
  <div>
    <PageHeaderCard
      v-if="showNewHeader"
      title="Empleados"
      subtitle="Administra usuarios, horarios, documentos y pagos."
      icon="people"
    >
      <template #actions>
        <q-btn
    ...
    <EmployeeFormDialog
      ...
      @job-position-change="onJobPositionChange"
        ...
        <div class="row items-center justify-between q-mb-md">
          <div>
            ...
            <div class="text-caption text-grey-7">
              Filtra empleados por compañía, departamento, puesto, proyecto y
              pago.
            </div>
          </div>
        ...
        <div class="row q-col-gutter-md">
          <div class="col-12 col-md-3">
            <q-select
              ...
              @update:model-value="onFilterCompanyChange"
            />
          </div>
          ...
          <div class="col-12 col-md-3">
            <q-select
              ...
              emit-value
              map-options
              @update:model-value="onFilterDepartmentChange"
            />
          </div>
          ...
          <div class="col-12 col-md-3">
            <q-select
              ...
              :options="filterJobPositions"
              option-label="name"
              option-value="_id"
              emit-value
              map-options
              @update:model-value="onFilterJobPositionChange"
            />
          </div>
          ...
          <div class="col-12 col-md-3">
            <q-input
              v-model="filters.hiringDateFrom"
              type="date"
              outlined
              dense
              rounded
    ...
    <Document ref="documento" @update:modelValue="getOperators" />
    <Contract ref="contrato" @update:modelValue="getOperators" />
    ...
    <UserSessionsDialog ref="userSessions" />
    ...
    <EmployeeVacationBalanceDialog
      ref="vacationDialog"
      :can-manage="canManageEmployees"
      @saved="() => getOperators({ withSearch: true })"
    />

    <EmployeeTerminationCreateDialog
      v-model="terminationDialog.open"
      :employee="terminationDialog.employee"
      @saved="onTerminationCreated"
    />
  </div>
</template>
...
import UserSessionsDialog from "src/components/employee/UserSessionsDialog.vue";
import EmployeeVacationBalanceDialog from "src/components/employee/vacation/EmployeeVacationBalanceDialog.vue";
import EmployeePasswordResetDialog from "src/components/employee/EmployeePasswordResetDialog.vue";
import EmployeeTerminationCreateDialog from "src/components/employeeTermination/EmployeeTerminationCreateDialog.vue";
...
const openPasswordDialog = (item) => {
  passwordResetDialog.value?.openModal(item);
};

//

const employeesTab = ref("active");
...
onMounted(async () => {
  await getAllData();
  await getOperators({ withSearch: true });
  await getRecruitmentApplicationsByDecision();
});
...
const getDepartment = async (companyId, targetRef = department) => {
  targetRef.value = [];

  const resp = await methodsHttp.getApi(`department/company/${companyId}`);

  targetRef.value = resp?.ok
...
const getOperators = async ({ withSearch = true } = {}) => {
  ...
  try {
    ...
    if (filters.value.company) q.set("company", filters.value.company);
    if (filters.value.department) q.set("department", filters.value.department);
    if (filters.value.jobPosition)
      q.set("jobPosition", filters.value.jobPosition);
    if (filters.value.project) q.set("project", filters.value.project);
    if (filters.value.salaryType) q.set("salaryType", filters.value.salaryType);
    if (filters.value.paymentSchedule) {
      q.set("paymentSchedule", filters.value.paymentSchedule);
    }
    if (filters.value.hiringDateFrom) {
      q.set("hiringDateFrom", filters.value.hiringDateFrom);
    }
    if (filters.value.hiringDateTo) {
      q.set("hiringDateTo", filters.value.hiringDateTo);
    }
...
const create = async () => {
  ...
  try {
    ...
    console.error("create employee modal error:", error);
    notify.value?.showNotifyBad("Error cargando los datos del formulario");
  } finally {
    formDataLoading.value = false;
  }
};

const hydrateEmployeeFormFromItem = (item) => {
  form.value.name = item.name || "";
  form.value.email = item.email || "";
  form.value.phone = item.phone || "";
  form.value.img = item.img || "";
  form.value.ext = item.ext || "";
  form.value.idNumber = item.idNumber || "";
  ...
  companySelected.value = item?.company?._id || null;
...
const openModalEdit = async (item) => {
  ...
  hydrateEmployeeFormFromItem(item);
  ...
  try {
    ...
    if (companySelected.value) {
      await getDepartment(companySelected.value);
    }

    if (companySelected.value && departmentSeleted.value) {
      await getJobPositions(companySelected.value, departmentSeleted.value);
    }

    if (
      companySelected.value &&
      departmentSeleted.value &&
    ...
    console.error("openModalEdit error:", error);
    notify.value?.showNotifyBad("Error cargando los datos del empleado");
  } finally {
    formDataLoading.value = false;
  }
};
...
const openVacationDialog = (item) => {
  vacationDialog.value?.openModal(item);
};

const openTerminationDialog = (item) => {
  if (!item?.company?._id && !item?.company) {
    notify.value?.showNotifyBad("El empleado no tiene compañía asignada.");
    return;
  }

  if (!item?.hiringDate) {
    notify.value?.showNotifyBad("El empleado no tiene fecha de contratación.");
    return;
  }

  terminationDialog.value = {
    open: true,
    employee: item,
  };
};

const onTerminationCreated = async () => {
  terminationDialog.value.open = false;
  terminationDialog.value.employee = null;

  notify.value?.showNotifyGood("Desvinculación creada correctamente.");
...
const save = async () => {
  try {
    ...
    const payload = {
      ...form.value,
      company: companySelected.value || null,
...
const openUserSessions = (item) => {
  userSessions.value?.openModal(item);
};
...
const onDepartmentChange = async (departmentId) => {
  ...
  await getJobPositions(companySelected.value, departmentId);
...
const onFilterCompanyChange = async (companyId) => {
  filters.value.department = null;
  filters.value.jobPosition = null;
  filters.value.project = null;

  filterDepartments.value = [];
  filterJobPositions.value = [];
  filterProjects.value = [];

  await getDepartment(companyId, filterDepartments);
  await search();
};

const onFilterDepartmentChange = async (departmentId) => {
  filters.value.jobPosition = null;
  filters.value.project = null;
...
const onFilterJobPositionChange = async (jobPositionId) => {
  filters.value.project = null;
  filterProjects.value = [];
...
const menuItemsFull = [
  ...
  },
  {
    ...
    text: "VACACIONES",
    color: "primary",
    action: openVacationDialog,
  },
  {
    icon: "devices",
    text: "SESIONES",
    color: "primary",
    action: openUserSessions,
  },
  {
    icon: "lock_reset",
