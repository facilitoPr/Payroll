    ...
    <div class="bg-white q-pa-sm">
        <div class="row q-col-gutter-sm">
            <div class="col-12 col-sm-auto col-md-auto row items-center">
                <q-btn color="primary" label="Crear zona" style="width: 100%; height: 40px" icon="add"
                    @click="create" />
            </div>
        </div>
                    ...
                    <q-card-section class="q-pt-sm">
                        <div class="q-pt-sm row q-col-gutter-sm">
                            ...
                            <div class="col-12">
                                <label>
                                    <b>
                                        <b class="text-negative">* </b> ENLANCE UBICACIÓN
                                    </b>
                                </label>
                                <q-input outlined dense color="primary" v-model="form.link">
                                </q-input>
                            </div>
                            ...
                            <div class="col-12 q-my-sm text-center">
                                ...
                                <q-btn type="submit" color="primary" label="guardar" icon="save" @click="save"
                                    :disable="form.name == '' || form.code == '' || form.link == ''" />
                            </div>
                        </div>
                    </q-card-section>
                </q-card>
        ...
        <!-- table -->
        <div>
            <q-table :loading="tableLoading" flat row-key="_id" title="Zonas" :rows="rows"
                ...
                <template v-slot:body="props">
                    <q-tr :props="props">
                        ...
                        ? props.row.img
    : 'https://plus-nautic.nyc3.digitaloceanspaces.com/pueblo.png'
        ...
        <Delete ref="deleteInfo" @deleteGood="getZones" />
    </div>
</template>
...
const getZones = async () => {
    ...
    if (resp.ok) {
        rows.value = resp.zones
        TableFilter.value = resp.zones
    }
...
const save = async () => {
    try {
        ...
        let resp;
        if (!mode.value) {
            resp = await methodsHttp.postApi("zones/createZone", { ...form.value });
        } else {
            resp = await methodsHttp.putApi(`zones/updateZone/${id.value}`, { ...form.value });
        }
