...
<template>
  <q-btn
    ...
    @click="
      () => {
        getClientsPaymentHistoryByOperator();
        openModal = true;
      }
    "
  >
    <q-tooltip anchor="top middle" self="bottom middle">
      <div style="font-size: 0.7rem">Historial de operador/@</div>
    </q-tooltip>
  </q-btn>
...
const getClientsPaymentHistoryByOperator = async () => {
  const resp = await methodsHttp.getApi(
    `client-payment-management/getClientsPaymentHistoryByOperator/${limit.value}/${initial.value}`
  );
  if (resp.ok) {
