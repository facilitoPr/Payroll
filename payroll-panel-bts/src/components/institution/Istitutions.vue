<template>
  <div>
    <div class="row q-col-gutter-sm">
      <div class="col-12 col-sm-4 col-md-4">
        <q-input
          outlined
          dense
          label="BUSCAR INSTITUCION"
          color="primary"
          @keyup="search"
          v-model="text"
        >
          <template v-slot:append>
            <q-icon name="search" color="primary" />
          </template>
        </q-input>
      </div>
      <div class="col-12 col-sm-auto col-md-auto row items-center">
        <q-btn
          color="primary"
          label="Agregar INSTITUCION"
          style="width: 100%; height: 40px"
          icon="add"
          @click="createDepartments"
        />
      </div>
    </div>

    <div class="q-my-md">
      <q-separator />
    </div>

    <!-- table -->
    <div>
      <q-table
        :rows="rows"
        :columns="columns.columnsMedicalInstitution()"
        title="INSTITUCIONES"
        row-key="name"
        :bordered="false"
        flat
        :rows-per-page-options="[10]"
        dense
      >
        <template v-slot:body="props">
          <q-tr :props="props">
            <q-td key="name" :props="props">
              <ImagenVue
                :img="
                  props.row.img
                    ? props.row.img
                    : 'https://adoscrila.nyc3.cdn.digitaloceanspaces.com/blankProfile.jpg'
                "
                :w="50"
                :h="50"
                style="cursor: pointer"
              />
            </q-td>
            <q-td key="name" :props="props">
              {{ props.row.name }}
            </q-td>
            <q-td key="name" :props="props">
              {{ props.row.email }}
            </q-td>

            <q-td key="name" :props="props">
              {{ props.row.type_of_institution.name }}
            </q-td>

            <q-td key="name" :props="props">
              {{ props.row.phone }}
            </q-td>

            <q-td key="name" :props="props">
              {{ props.row.rnc }}
            </q-td>

            <q-td key="name" :props="props">
              <div v-if="props.row.isActive">
                <q-badge
                  color="secondary"
                  label="ACTIVO"
                  style="cursor: pointer"
                />
              </div>
              <div v-else>
                <q-badge
                  color="negative"
                  label="INACTIVO"
                  style="cursor: pointer"
                />
              </div>
            </q-td>

            <q-td key="name" :props="props">
              <q-icon
                name="menu"
                color="primary"
                size="25px"
                style="cursor: pointer"
              >
                <q-menu
                  transition-show="flip-right"
                  transition-hide="flip-left"
                >
                  <!-- editar -->
                  <q-list
                    style="min-width: 100px"
                    @click="openModalEdit(props.row)"
                  >
                    <q-item clickable class="row items-center">
                      <div class="col-auto">
                        <q-icon name="edit" color="primary" size="25px" />
                      </div>
                      <div class="q-px-sm col-auto">EDITAR</div>
                    </q-item>
                  </q-list>
                  <!-- eliminar -->
                  <q-list
                    style="min-width: 100px"
                    @click="openModalDelete(props.row)"
                  >
                    <q-item clickable class="row items-center">
                      <div class="col-auto">
                        <q-icon name="delete" color="negative" size="25px" />
                      </div>
                      <div class="q-px-sm col-auto">ELIMINAR</div>
                    </q-item>
                  </q-list>
                  <!-- imagen -->
                </q-menu>
              </q-icon>
              <!-- <q-btn dense icon="menu"> </q-btn> -->
            </q-td>
          </q-tr>
        </template>
      </q-table>
    </div>

    <!-- modal -->
    <div>
      <q-dialog v-model="openModal" persistent>
        <q-card style="width: 900px; max-width: 90vw">
          <div class="bg-white row justify-between q-pa-md">
            <div class="text-white" style="font-size: 19px; font-weight: 500">
              <div class="row items-center">
                <div class="col-auto q-mx-sm">
                  <q-icon size="2em" name="add" color="primary" />
                </div>
                <div class="col-auto text-primary">
                  <b>{{
                    modo ? "EDITAR DEPARTAMENTO" : "AGREGAR DEPARTAMENTO"
                  }}</b>
                </div>
              </div>

              <!-- groups -->
            </div>
            <span
              class="material-icons text-negative"
              style="font-size: 23px; cursor: pointer"
              @click="openModal = false"
            >
              cancel
            </span>
          </div>
          <q-form class="q-gutter-md">
            <q-card-section class="q-pt-sm q-mx-xl">
              <div class="q-pt-sm row q-col-gutter-sm">
                <div class="col-12 col-md-6">
                  <label>
                    <b> <b class="text-negative">* </b> TIPO DE INSTITUCION </b>
                  </label>
                  <!-- <q-input  outlined dense color="primary" v-model="form.name">
                </q-input> -->
                  <q-select
                    outlined
                    dense
                    color="primary"
                    :options="typeOfInstitution"
                    v-model="typeOfInstitutionSelected"
                    option-label="name"
                  >
                    <template v-slot:append>
                      <q-icon name="event" color="orange" />
                    </template>
                  </q-select>
                </div>
                <div class="col-12 col-md-6">
                  <label>
                    <b> <b class="text-negative">* </b> NOMBRE </b>
                  </label>
                  <q-input outlined dense color="primary" v-model="form.name">
                    <template v-slot:append>
                      <q-icon name="event" color="orange" />
                    </template>
                  </q-input>
                </div>

                <div class="col-12 col-md-6">
                  <label>
                    <b> <b class="text-negative">* </b> EMAIL </b>
                  </label>
                  <q-input
                    outlined
                    dense
                    color="primary"
                    type="mail"
                    v-model="form.email"
                  >
                  </q-input>
                </div>

                <div class="col-12 col-md-6">
                  <label>
                    <b>
                      <b class="text-negative">* </b> TELÉFONO DE SERVICIO
                    </b>
                  </label>
                  <q-input
                    outlined
                    dense
                    color="primary"
                    mask="(###) ### - ####"
                    fill-mask
                    type="tel"
                    v-model="form.phone"
                  />
                </div>

                <div class="col-12 col-md-6">
                  <label>
                    <b> <b class="text-negative">* </b> RNC </b>
                  </label>
                  <q-input outlined dense color="primary" v-model="form.rnc" />
                </div>
                <div class="col-12 col-md-6">
                  <label>
                    <b> <b class="text-negative">* </b> ACTIVO </b>
                  </label>
                  <q-checkbox v-model="form.isActive" />
                </div>

                <div class="col-12 col-md-12">
                  <label>
                    <b> <b class="text-negative">* </b> ADDRESS </b>
                  </label>
                  <q-input
                    type="textarea"
                    outlined
                    dense
                    color="primary"
                    v-model="form.address"
                  >
                  </q-input>
                </div>

                <div class="col-12 q-my-sm text-center">
                  <q-btn
                    type="submit"
                    color="negative"
                    label="cancelar"
                    icon="cancel"
                    class="q-mx-sm"
                    @click="openModal = false"
                  />

                  <q-btn
                    type="submit"
                    color="primary"
                    label="guardar"
                    icon="save"
                    @click="save"
                    :disable="form.name == '' || form.code == ''"
                  />
                </div>
              </div>
            </q-card-section>
          </q-form>
        </q-card>
      </q-dialog>
    </div>
    <NotificationsVue ref="notify" />
    <Delete ref="deleteInfo" @deleteGood="getMedicalInstitution" />
  </div>
</template>
<script setup>
import { onMounted, ref, watch } from "vue";
import methodsHttp from "src/api/methodsHttp";
import columns from "src/components/utils/columns";
import NotificationsVue from "src/components/utils/Notifications.vue";
import Delete from "src/components/utils/Delete.vue";
import ModifiText from "src/components/utils/modifiText";

import ImagenVue from "src/components/utils/Imagen.vue";

const notify = ref();
const deleteInfo = ref();

const openModal = ref(false);
const modo = ref(false);

const form = ref({
  name: "",
  address: "",
  phone: "",
  rnc: "",
  email: "",
  type_of_institution: "",
  isActive: true,
});

const typeOfInstitution = ref([]);
const typeOfInstitutionSelected = ref(null);

watch(typeOfInstitutionSelected, (value) => {
  if (value != null) {
    form.value.type_of_institution = value._id;
  }
});

const text = ref("");
const rows = ref([]);
const TableFilter = ref([]);

const tableLoading = ref(false);

const reset = () => {
  form.value.name = "";
  form.value.address = "";
  form.value.phone = "";
  form.value.rnc = "";
  form.value.email = "";
  form.value.type_of_institution = "";
  typeOfInstitutionSelected.value = null;
};

const id = ref(null);

const openModalEdit = (item) => {
  form.value.name = item.name;
  form.value.address = item.address;
  form.value.phone = item.phone;
  form.value.rnc = item.rnc;
  form.value.email = item.email;
  form.value.isActive = item.isActive;
  typeOfInstitutionSelected.value = item.type_of_institution;
  modo.value = true;
  openModal.value = true;
  id.value = item._id;
};

onMounted(() => {
  getMedicalInstitution();
  getTypeOfInstitution();
});

const getTypeOfInstitution = async () => {
  let resp = await methodsHttp.getApi(
    `medical/institution/getTypeOfInstitution`
  );
  if (resp.ok) {
    typeOfInstitution.value = resp.typeOfInstitution;
  }
};

const getMedicalInstitution = async () => {
  tableLoading.value = true;
  let resp = await methodsHttp.getApi(
    `medical/institution/getMedicalInstitution`
  );
  if (resp.ok) {
    rows.value = resp.medical;
    TableFilter.value = resp.medical;
  }
  tableLoading.value = false;
};

const createDepartments = () => {
  reset();
  modo.value = false;
  openModal.value = true;
};

const openModalDelete = (item) => {
  const data = {
    id: item._id,
    ruta: "medical/institution/deleteMedicalInstitution",
  };

  deleteInfo.value?.openDelete(data, "MedicalInstitution");
};

const save = async () => {
  if (!modo.value) {
    let resp = await methodsHttp.postApi(
      "medical/institution/create",
      form.value
    );
    if (resp.ok) {
      openModal.value = false;
      notify.value?.showNotifyGood(resp.mensaje);
      getMedicalInstitution();
    } else {
      notify.value?.showNotifyBad(resp.mensaje);
    }
  } else {
    let resp = await methodsHttp.putApi(
      `medical/institution/putMedicalInstitution/${id.value}`,
      form.value
    );
    if (resp.ok) {
      openModal.value = false;
      notify.value?.showNotifyGood(resp.mensaje);
      getMedicalInstitution();
    } else {
      notify.value?.showNotifyBad(resp.mensaje);
    }
  }
};

const search = () => {
  let textM = ModifiText.ModifiText(text.value);
  if (textM != "") {
    const filter = TableFilter.value.filter((e) => {
      return ModifiText.ModifiText(e.name).includes(textM);
    });
    console.log(filter);
    rows.value = filter;
  } else {
    rows.value = TableFilter.value;
  }
};
</script>
