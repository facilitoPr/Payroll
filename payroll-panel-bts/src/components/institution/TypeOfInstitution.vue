<template>
  <div>
    <div class="row q-col-gutter-sm">
      <div class="col-12 col-sm-3 col-md-4">
        <q-input
          outlined
          dense
          label="BUSCAR TIPO DE INSTITUCION"
          color="primary"
          @keyup="search"
          v-model="text"
        >
      ...
      <div class="col-12 col-sm-auto col-md-auto row items-center">
        <q-btn
          color="primary"
          label="Agregar TYPO DE INSTITUCION"
    ...
    <div>
      <q-table
        :rows="rows"
        :columns="columns.columnsTypeOfInstitution()"
      ...
      >
        <template v-slot:body="props">
          <q-tr :props="props">
            ...
            <q-td key="name" :props="props">
              <div v-for="(item, index) in props.row.departments" :key="index">
               {{ item.name }}
              </div>
            </q-td>
            <q-td key="name" :props="props">
          ...
          <q-card-section class="q-pt-sm">
            <div class="q-pt-sm row q-col-gutter-sm">
              ...
              <div class="col-12 col-md-12">
                <label>
                  <b>
                    <b class="text-negative">* </b> NOMBRE DEL TIPO DE
                    INSTITUCION
                  </b>
                </label>
    ...
    <Delete ref="deleteInfo" @deleteGood="getTypeOfInstitution" />
  </div>
</template>
<script setup>
...
const openModalEdit = (item) => {
  ...
  departmentsSelected.value = item.departments;
  modo.value = true;
...
const getTypeOfInstitution = async () => {
  ...
  let resp = await methodsHttp.getApi(`medical/institution/getTypeOfInstitution`);
  if (resp.ok) {
    rows.value = resp.typeOfInstitution;
    TableFilter.value = resp.typeOfInstitution;
  }
...
const save = async () => {
  form.value.departments = []
  departmentsSelected.value.map((e) => {
    form.value.departments.push(e._id);
  });

  if (!modo.value) {
    let resp = await methodsHttp.postApi(
      "medical/institution/createTypeOfInstitution",
      form.value
    );
    if (resp.ok) {
    ...
    let resp = await methodsHttp.putApi(
      `medical/institution/putTypeOfInstitution/${id.value}`,
      form.value
    );
