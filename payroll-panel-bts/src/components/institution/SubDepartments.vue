<template>
  <div>
    <div class="row q-col-gutter-sm">
      ...
      <div class="col-12 col-sm-3 col-md-3">
        <q-select
          ...
          color="primary"
          :options="departments"
          v-model="departmentsSelectedFilter"
          option-label="name"
          label="DEPARTAMENTOS"
        >
        </q-select>
      ...
      <div class="col-12 col-sm-auto col-md-auto row items-center">
        <q-btn
          color="primary"
          label="Agregar subdepartamentos"
    ...
    <div>
      <q-table
        :rows="rows"
        :columns="columns.columnsSubdepartamentos()"
            ...
            <div class="text-white" style="font-size: 19px; font-weight: 500">
              <div class="row items-center">
                ...
                <div class="col-auto text-primary">
                  <b>{{
                    modo ? "EDITAR SUBDEPARTAMENTO" : "AGREGAR SUBDEPARTAMENTO"
                  }}</b>
                </div>
              </div>
          ...
          <q-card-section class="q-pt-sm">
            <div class="q-pt-sm row q-col-gutter-sm">
              ...
              <div class="col-12 col-md-12">
                <label>
                  <b>
                    <b class="text-negative">* </b> NOMBRE DEL SUBDEPARTAMENTO
                  </b>
                </label>
    ...
    <Delete ref="deleteInfo" @deleteGood="getSubdepartments" />
  </div>
</template>
<script setup>
...
const getSubdepartments = async () => {
  ...
  let resp = await methodsHttp.getApi(`medical/institution/getSubdepartments`);
  if (resp.ok) {
    rows.value = resp.subdepartments;
    TableFilter.value = resp.subdepartments;
  }
...
const openModalDelete = (item) => {
  const data = {
    id: item._id,
    ruta: "medical/institution/deleteSubdepartments",
  };

  deleteInfo.value?.openDelete(data, "Subdepartments");
};

const save = async () => {
  if (!modo.value) {
    let resp = await methodsHttp.postApi(
      "medical/institution/createSubDepartments",
      form.value
    );
    if (resp.ok) {
    ...
    let resp = await methodsHttp.putApi(
      `medical/institution/putSubdepartments/${id.value}`,
      form.value
    );
